...
const uint16_t NRegionsPerLink =
    7;  // Bits 8-21, ..., 104-119, keeping ranges (7, 0) and (127, 120) unused
const uint16_t MaxRegions = N_CH_IN * NRegionsPerLink;

/*
 * algo_unpacked interface exposes fully unpacked input and output link data. This version assumes
 * use of 10G 8b10b links, and thus providing 192bits/BX/link.
 *
 * !!! N.B.: Do NOT use the first and the last bytes of link_in (i.e. link_in.range(7,0) and
 * link_in.range(127, 120) as these fields are reserved for transmission of 8b10b input/output link
 * alignment markers and CRC checksum word. The remaining 112 bits are available for algorithm use.
 *
 * !!! N.B. Make sure to assign every bit of link_out[] data.
 */
...
// null algo specific pragma: avoid fully combinatorial algo by specifying min latency otherwise
// algorithm clock input (ap_clk) gets optimized away
#pragma HLS latency min = 4
    ...
    //  pass-through "algo"
           ...
           link_out[lnk].range(119, 8) = link_in[lnk].range(119, 8) ;   // input to output
    pass-through
        }
    */
...
regionLoop:
    for (int iRegion = 0; iRegion < NR_CNTR_REG; iRegion++) {
#pragma HLS UNROLL
...
#pragma HLS UNROLL
    ...
    boostedjet(algo_config.jet_seed, centr_region_pu_sub, et_3by3_calo, et_jet_boosted,
               rIdx_boostedjet);
...
#pragma HLS UNROLL
        /* Boosted jets
           ...
           collection is 8 bits phi then 8 bits in eta then 11 bits et and 1 bit for flag.
        */
        ap_uint<9> idx_srt;
